...
# # train the VSFA model with sparsely sampling strategy at certain interval
...
from folders_updated import BTFVVFolder, BTFVVFolderVSFA, BTFVVFolderInterval, BTFVVFolderIntervalTSN
...
class TrainManager(object):
    ...
    def __init__(self, data, options):
        ...
        if self._options['vsfa']:
            ...
            print("running by others")  # BTFVVFolder also works
            self._train_data = BTFVVFolderIntervalTSN(self._data['data_path'], self._data['vsfa_feature_path'],
                                                   self._options['iter_number'], self._options['frame_number'],
                                                   self._options['interval_number'], True)
            self._train_data_loader = torch.utils.data.DataLoader(self._train_data, batch_size=self._options["batch_size"],
                                                                  shuffle=True)
            self._test_data = BTFVVFolderIntervalTSN(self._data['data_path'], self._data['vsfa_feature_path'],
                                                  self._options['iter_number'], self._options['frame_number'],
                                                  self._options['interval_number'], False)
            self._test_data_loader = torch.utils.data.DataLoader(self._test_data, 1, shuffle=False)
    ...
    def train(self):
        ...
        best_srcc = 0
        model_path = os.path.join(self._data['para_saving_path'],
                                  (str(self._options['iter_number']) + '_' + str(self._options['frame_number'])
                                   + '_' + 'tsn' + '.pkl'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vsfa', dest='vsfa', type=str2bool,
                        default=False, help='Truth represents train VSFA, while others denote other options.')
    ...
    parser.add_argument('--iter_number', dest='iter_number', type=int,
                        default=0, help='Inters for training')
    ...
    parser.add_argument('--interval_number', dest='interval_number', type=int,
                        default=4, help='Interval number of sparse sampling')
    ...
    if args.vsfa:
        ...
        print(data['para_saving_path'])
    else:
        data['para_saving_path'] = args.para_saving_path + "_Others"
    data['per_save_file'] = os.path.join(data['para_saving_path'],
                                         'performance_'+str(args.iter_number)+'_'+str(args.frame_number)+'_'+'tsn'+'.txt')
